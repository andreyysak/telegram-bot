...
import { DELETE_TEXT, todoKeyboard } from "../../keyboard/todoKeyboard.js";
import { BACK_TO_MAIN_TEXT } from "../../keyboard/backToMenu.js";
import { mainMenuKeyboard } from "../../keyboard/mainMenu.js";
import pool from "../../db/client.js";

export const deleteTodoModule = new Composer<BotContext>()

deleteTodoModule.hears(DELETE_TEXT, async (ctx) => {
...
deleteTodoModule.hears(BACK_TO_MAIN_TEXT, async (ctx) => {
...
deleteTodoModule.on(':text').filter(
  (ctx): boolean => ctx.session.todo?.state === 'delete_todo',
  async (ctx) => {
    ...
    try {
      ...
      // 🔥 Видаляємо справу з бази
      await pool.query(
        'DELETE FROM todos WHERE id = $1 AND user_id = $2',
        [todoId, dbUserId]
      );

      await ctx.reply(`🗑️ Справу "${todoText}" успішно видалено.`, {
        reply_markup: todoKeyboard,
      });
